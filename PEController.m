...
// -------------------------------------------------------------------------
// Created: 2. June 2003
// Version: 21. November 2004 18:38
// =========================================================================
...
// Version: 14 September 2006 21:46
// =========================================================================
...
- (void) erase
{
	...
	{
		if ([[NSWorkspace sharedWorkspace] isFilePackageAtPath: [trash_files objectAtIndex: j]] == YES ) // if this file is a package, find the number of files contained within
		{
...
// Version: 6 March 2007 20:10
// =========================================================================
- (void) selectNextFile
{
	if ([self isVolume: [trash_files objectAtIndex: i]] == YES && [self isErasableDisc: [trash_files objectAtIndex: i]])
	{
		[self eraseDisc];
	}
	else
...
{	
	...
	device = [DRDevice deviceForBSDName: [self bsdDevNode: [trash_files objectAtIndex: i]]];
...
- (void) runTask
{
    ...
    if ([self isFileSymbolicLink: [trash_files objectAtIndex: i]] == YES)
    {
        [pEraser setLaunchPath:@"/bin/rm"];
        [pEraser setArguments: [NSArray arrayWithObjects: @"-Pv", [trash_files objectAtIndex: i], nil] ];
    }
    else // regular file or directory
    {
        [pEraser setLaunchPath:util_path];
        [pEraser setArguments: [NSArray arrayWithObjects: @"-fvrz", [trash_files objectAtIndex: i], nil] ];
    }
	
	
	// Throw a warning if a file cannot be erased
	if ([fm isDeletableFileAtPath:[trash_files objectAtIndex: i]] == NO || 
		[self checkPermissions: [trash_files objectAtIndex: i]] == NO)
	{
	...
	else
	{
		...
		NSDictionary *fileAttributes = [fm fileAttributesAtPath:[trash_files objectAtIndex: i] traverseLink:NO];
		...
		{
			[fm changeFileAttributes: [NSDictionary dictionaryWithObject:[NSNumber numberWithBool:NO] forKey:NSFileImmutable] atPath:[trash_files objectAtIndex: i]];
		}
		
		// If it is a directory or empty file, add 100 since it doesn't delete like normal files
		if (([fm fileExistsAtPath: [trash_files objectAtIndex: i] isDirectory:&isDir] && isDir &&
			 [[NSWorkspace sharedWorkspace] isFilePackageAtPath: [trash_files objectAtIndex: i]] == NO) )
		{
...
- (NSString *) currentFileName
{
	return ([[trash_files objectAtIndex: i] lastPathComponent]);
}
...
// Version: 10. January 2007 20:00
// =========================================================================
- (NSString *) fileNameString
{
    NSMutableString *current_file_name = [[NSMutableString alloc] initWithString: [[trash_files objectAtIndex: i] lastPathComponent]];
...
- (void) doneErasing: (NSNotification *)aNotification 
{
    i++; // increment the counter here to prevent out of bound array errors
	
    if (i >= num_files)  // Jobs are complete.  Quit the app.
    {
...
// Version: 10 December 2006 22:10
// =========================================================================
- (IBAction) cancelErasing: (id) sender
{
    i = num_files;
...
- (void) shutdownPE
{
	...
	{
		if ( i == 0 && [fm fileExistsAtPath: @"/System/Library/Sounds/Funk.aiff"] == YES ) // If there are no files to erase
		{
